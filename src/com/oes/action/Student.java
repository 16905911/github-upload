...
public class Student extends Action {
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form,
		...
		System.out.println("Student获取的查询字符串：" + action);
	...
	public ActionForward studentLogin(ActionMapping mapping, ActionForm form,
		...
		System.out.println("studentLogin验证结果ret的值:" + ret);
		if (ret == 2) {
			request.setAttribute("error", "您输入的考生准考证号码或密码错误！");
			return mapping.findForward("error");
		} else {
			HttpSession session = request.getSession();
			session.setAttribute("student", studentForm.getID());  //获取登陆学生的ID
			System.out.println("studentID= " + studentForm.getID());
	...
	private ActionForward studentAdd(ActionMapping mapping, ActionForm form,
		...
		System.out.println("studentAdd返回值ret：" + ret);
		if (ret.equals("re")) {
	...
	private ActionForward modifyQuery(ActionMapping mapping, ActionForm form,
			HttpServletRequest request, HttpServletResponse response) {
		System.out.println("modifyQuery获取的ID："+request.getParameter("ID"));
	...
	private ActionForward seekPwd2(ActionMapping mapping, ActionForm form,
		...
		if(s.getCardNo().equals("")){
			request.setAttribute("error", "您输入的该提示问题的答案不正确！");
			return mapping.findForward("error");
		}else{
