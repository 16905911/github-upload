...
public class StartExamDAO {
    ...
    public int randomGetQuestion(int lessonID){
        ...
        try {
            ...
            int rand=Math.abs(new Random().nextInt(id.length)); //随机获取试卷ID
            questionsID=id[rand];
        } catch (Exception ex) {
        	ex.printStackTrace();
        }    	
    ...
    public List queryExam(int questionsID,int flag){
        ...
        String sql="";
        if(flag==0){
            sql = "SELECT * FROM questionsinfo WHERE taoTiID="+questionsID+" AND type='单选题'";
        }else{
        	sql = "SELECT * FROM questionsinfo WHERE taoTiID="+questionsID+" AND type='多选题'";
        }
        ResultSet rs = conn.executeQuery(sql);
        String type="";
        int id=0;
